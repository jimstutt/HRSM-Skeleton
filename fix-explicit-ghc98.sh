...
# 1. Update flake.nix to explicitly use _9_8 packages
  ...
  outputs = { self, nixpkgs, flake-utils, ghc-wasm-meta }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Explicitly use the 9.8 versions
      ...
      in {
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = '' 
            echo "[HRSM] Dev shell loaded. Wasm Compiler: wasm32-wasi-ghc-9_8"; 
          '';
        };
      }
    );
...
# 2. Update build-wasm.sh to use explicit _9_8 binaries
...
wasm32-wasi-cabal-9_8 build frontend-wasm-exe \
