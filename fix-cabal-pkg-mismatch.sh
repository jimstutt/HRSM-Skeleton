...
echo "[HRSM] Fixing cabal ghc-pkg version mismatch..."
...
echo "[2/3] Building frontend-wasm with cabal (GHC 9.8 + external interpreter)..."
export HOME="$DIR"
export CABAL_DIR="$DIR/.cabal"
rm -rf "$HOME/.config/cabal"

# Unset GHC_PACKAGE_PATH to prevent cabal from using the system package DB
unset GHC_PACKAGE_PATH

cabal update

# Explicitly specify all wasm toolchain binaries to prevent fallback to system GHC
