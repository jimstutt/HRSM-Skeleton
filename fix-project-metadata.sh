...
echo "[HRSM] Correcting project metadata to HRSM-Skeleton + MariaDB..."

# 1. Update main.ts with correct project name and DB info
...
sed -i "s|<title>.*</title>|<title>HRSM-Skeleton</title>|" "$DIR/frontend/index.html"

# 3. Commit corrections
cd "$DIR"
git add frontend/src/main.ts frontend/index.html
git commit -m "[HRSM] Fix project metadata: HRSM-Skeleton + MariaDB (was NGO Logistics CG + SQLite)" || true

echo "[HRSM] ✓ Metadata corrected. Vite HMR should auto-reload."
