...
echo "[HRSM] Updating flake.nix to use wasm32-wasi-sdk for proper Wasm linking..."
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Import the project's custom WASI SDK
        wasmSdk = pkgs.callPackage ./pkgs/wasm32-wasi-sdk.nix {};
        ...
        frontendWasmPkg = pkgs.stdenv.mkDerivation {
          ...
          nativeBuildInputs = [
            pkgs.ghc
            wasmSdk  # Provides clang, wasm-ld, and WASI libraries
          ];

          buildPhase = ''
            echo "[HRSM] Building Wasm frontend via scripts/build-wasm.sh"
            # Ensure wasm-sdk's bin directory is first in PATH
            export PATH="${wasmSdk}/bin:$PATH"
