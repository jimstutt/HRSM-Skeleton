...
echo "[HRSM] Updating flake.nix to use Nix's llvmPackages.clang instead of custom WASI SDK..."
  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # 3. Build 'frontend-wasm' using Nix's clang with wasm target support
        frontendWasmPkg = pkgs.stdenv.mkDerivation {
          ...
          nativeBuildInputs = [
            ...
            pkgs.llvmPackages.lld  # provides wasm-ld
          ];
...
echo "[HRSM] flake.nix updated to use Nix's llvmPackages."
